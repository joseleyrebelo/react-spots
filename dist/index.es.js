import require$$0, { useState, useRef, useCallback, useEffect, createContext } from "react";
...
function useStateCallback(initialState) {
  ...
  const setStateCallback = useCallback((state2, cb) => {
    cbRef.current = cb;
    setState(state2);
  }, []);
  useEffect(() => {
...
const newSpot = (values, methods) => {
  ...
  const useContext = () => require$$0.useContext(context);
  ...
  return ({
    ...
    objKeys(props).forEach((key) => {
      if (values.states && key in values.states)
        ...
        values.middleware[key] = props[key];
    });
...
const shapeData = (values, methods, withState = false) => {
  ...
  return {
    ...values,
    ...methods,
    ...states && objKeys(states).reduce((aggregate, key) => {
